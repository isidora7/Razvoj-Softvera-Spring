...
public interface StudentIndeksRepository extends JpaRepository<StudentIndeks, Long> {
	...
	@Query("select si from StudentIndeks si where si.student.id = :idStudentPodaci and si.aktivan = true order by si.id desc")
	List<StudentIndeks> findAktivniStudentIndeksiByStudentPodaciId(Long idStudentPodaci);

	@Query("select si from StudentIndeks si where si.student.id = :studentId and si.studProgramOznaka = :oznaka and si.godina = :godina order by si.id desc")
	List<StudentIndeks> findExistingForStudentAndProgramAndYear(@Param("studentId") Long studentId,
