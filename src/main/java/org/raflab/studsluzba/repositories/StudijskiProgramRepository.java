...
//
	...
	List<StudijskiProgram> findByOznaka(String oznaka);
...
//
//	@Query("select p from Predmet p join StudijskiProgram sp on p.studProgram.id = sp.id where sp.id = :studijskiProgramId")
//	List<Predmet> getPredmetsInStudijskiProgram(Long studijskiProgramId);


	//@Query("select p from Predmet p where p.studProgram.id = :studijskiProgramId")
	//List<Predmet> getPredmetsInStudijskiProgram(Long studijskiProgramId);
	//
}